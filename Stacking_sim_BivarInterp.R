...
## rerun FishLife model with new values
...
# saveRDS(res_all, file.path(res_dir, "summary_RE_Cover.rds"))
...
cover <- summary %>% filter(Year==max(Year, na.rm=TRUE)) %>%
       group_by(Approach, Population, Model, Variable) %>%
       select(Approach, Population, Model, Variable, Cover) %>%
       summarise_all(funs(Coverage=sum(., na.rm=TRUE), Converge=length(which(is.na(Cover)==FALSE)))) %>%
...
pcover <- ggplot(cover %>% filter(Variable=="SPR")) +
    geom_point(aes(x=Approach, y=PropCover, color=Population, fill=Population, shape=Population), cex=6, alpha=0.7) +
    ...
    ylim(c(0,1))
ggsave(file.path(fig_dir, "Cover_SPR.png"), height=8, width=7, pcover)
