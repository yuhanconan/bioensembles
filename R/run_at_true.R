run_at_true <- function(modpath, iter, lh, model){
	...
	if(model=="LBSPR"){
		...
		saveRDS(lbspr_res, file.path(iterpath, paste0("res_BestCase_LBSPR.rds")))
	}
	...
	if(model=="LIME"){
		...
		if(all(is.null(out$df))){
			write("model NA", file.path(iterpath, "modelNA_BestCase_LIME.txt"))
		}
		if(all(is.null(out$df))==FALSE){
			gradient <- out$opt$max_gradient <= 0.001
			pdHess <- out$Sdreport$pdHess
			if(gradient==FALSE){
				write("highgradient", file.path(iterpath,"highgradient_BestCase_LIME.txt"))
			}
			if(pdHess==FALSE){
				write("Hessian not positive definite", file.path(iterpath, "pdHess_BestCase_LIME.txt"))
			}
			## save results if converged
			if(gradient == TRUE & pdHess == TRUE) saveRDS(out, file.path(iterpath, paste0("res_BestCase_LIME.rds")))	
		}	
	}
}