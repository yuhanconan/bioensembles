run_at_means <- function(modpath, iter, lh, model){
	...
	if(model=="LBSPR"){
		...
		saveRDS(lbspr_res, file.path(iterpath, paste0("res_Means_LBSPR.rds")))
	}
	...
	if(model=="LIME"){
		...
		if(isNA==FALSE){
			...
			if(pdHess==FALSE | gradient==FALSE){
				input$theta <- 50
		...
		if(all(is.null(out$df))){
			write("model NA", file.path(iterpath, "modelNA_Means_LIME.txt"))
		}
		if(all(is.null(out$df))==FALSE){
			gradient <- out$opt$max_gradient <= 0.001
			pdHess <- out$Sdreport$pdHess
			if(gradient==FALSE){
				write("highgradient", file.path(iterpath,"highgradient_Means_LIME.txt"))
			}
			if(pdHess==FALSE){
				write("Hessian not positive definite", file.path(iterpath, "pdHess_Means_LIME.txt"))
			}
			## save results if converged
			if(gradient == TRUE & pdHess == TRUE) saveRDS(out, file.path(iterpath, paste0("res_Means_LIME.rds")))	
		}	
	}
}